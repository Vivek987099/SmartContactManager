...
import { MdOutlineDeleteOutline } from "react-icons/md";import { IoCallOutline } from "react-icons/io5";
import { MdOutlineEmail } from "react-icons/md";


function ContactCard({ setToggle,currentContact}) {
  return (
    <div
     
      className=" bg-gray-50 p-5 h-auto w-110 absolute top-18 left-[30%] rounded shadow-2xl pb-10"
    >
      <div className="flex justify-between items-center text-2xl m-1">
        ...
        <div className="flex gap-x-5">
          ...
          <div>
            <MdOutlineDeleteOutline className="cursor-pointer text-red-600"/>
          </div>
        </div>
      </div>
      <div  className="flex flex-col items-center m-10">
        <div className="">
          <img src={"http://localhost:8080/images/" + currentContact.image} className="size-30 rounded-full" alt="" />
        </div>
        <h1 className="font-semibold mt-3 text-2xl mb-2">{currentContact.name+" "+currentContact.secondName}</h1>
      ...
      <div className="my-3"><span className="font-semibold">Work :</span>
        <p className="inline ml-2 capitalize" >{currentContact.work}</p>
      </div>
      <p> <span className="font-semibold">About :</span> {currentContact.description}</p>
